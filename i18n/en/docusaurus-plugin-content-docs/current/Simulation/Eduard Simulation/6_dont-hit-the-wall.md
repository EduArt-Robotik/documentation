...
title: Don't hit the Wall Examples
sidebar_position: 6
---
# Don't Hit the Wall Examples

Prerequisites:
- Container is running
- VSC connected to the container
- "Open Folder" → ros2_ws from the container

## Python 

- Create a file in ros2_ws named `dont-hit-the-wall.py`
...
- Then open a terminal in http://localhost:8080/vnc.html or enter `start-simulation.sh` to start the simulation (I choose option 2)
- Stop the Virtual Joystick in the terminal at the bottom left (ctrl + c)
- Instead, enter the following command in the workspace 

```bash
ros2 run edu_robot_control_template dont_hit_the_wall_node.py --ros-args -r __ns:=/eduard/blue
```

- Don't forget to click "Start" in the simulation – then the Eduard robot will move straight until it is close to the wall.


--- 

ToDo:
- Examples for manually controlling the robot in different directions before "Don't hit the wall", Hannes' list

- Represent directions of the robot, x, y, and Z, explain the right-hand rule
- Terminal navigation with arrow keys is cumbersome, consider using the terminal instead 

subscribe to Range sensors, publish to driving
